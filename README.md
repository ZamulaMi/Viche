...
знаходять одне одного, чат — через DataConnection. Перший клієнт у пулі
тримає чергу («лобі»), за потреби відбувається автоматичне підвищення.

Медіа йде **тільки глобально**: `iceTransportPolicy: "relay"` — жодних
LAN-кандидатів, уся медіа через TURN (Metered Open Relay + резервний),
тож пари з'єднуються за будь-яким NAT, у будь-якій мережі. Власний TURN
(coturn з compose): `VITE_TURN_URL=turn:ваш-хост:3478 VITE_TURN_USERNAME=…
VITE_TURN_CREDENTIAL=… npm run build`. Якщо брокер недоступний — м'який
фолбек на офлайн-демо з симульованим співрозмовником:
